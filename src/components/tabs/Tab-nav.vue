<script>
  ...
  export default {
    ...
    props: {
      panes: Array,
      currentName: [Number, String],    // 兼容 sunflower2.0.0 以下版本， 扩充currentName类型可为Number
      editable: Boolean,
      onTabClick: {
        type: Function,
        default: noop
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['s-tabs-nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="iconfont icon-left"></i></span>,
          <span class={['s-tabs-nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="iconfont icon-right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        // pane.index = `${index}`;
        /* 兼容sunflower2.0.0以前版本，panel.index值为Number类型*/
        pane.index = index;

        const btnClose = closable
          ? <span class="iconfont icon-error" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['s-tabs-nav-wrap', scrollable ? 'is-scrollable' : '']}>
          {scrollBtn}
          <div class={['s-tabs-nav-scroll']} ref="navScroll">
            <div class="s-tabs-nav" ref="nav" style={navStyle} role="tablist" on-keydown={ changeTab }>
