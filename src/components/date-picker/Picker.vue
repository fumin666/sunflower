<template>
  ...
  <div
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['s-input-icon', 's-range-icon', 'iconfont', 'icon-' + triggerClass]"></i>
    <input
...
export default {
  ...
  data() {
    return {
      ...
      // valueFormat: this.type === 'week' ? null : DEFAULT_FORMATS[this.type]
    };
  },
  ...
  watch: {
    pickerVisible(val) {
      ...
      if (val) {
        ...
        this.dispatch('SFormItem', 'el.form.blur');
        this.blur();
      }
    },
    parsedValue: {
