<template>
    ...
    <slot>
      ...
      <span v-show="itemSelected" class="s-select-selected-check">
...
<script>
  ...
  export default {
    ...
    watch: {
      currentLabel() {
        if (!this.created && !this.select.remote) this.dispatch('SSelect', 'setSelected');
      },
      value() {
        if (!this.created && !this.select.remote) this.dispatch('SSelect', 'setSelected');
      }
