<template>
  <div
    class="s-tree"
    :class="{
      's-tree-highlight-current': highlightCurrent,
    ...
    <s-tree-node
      ...
      :islast="index==root.childNodes.length-1"
  ...
  export default {
    ...
    methods: {
      ...
      handelKeydown(ev) {
        ...
        if (currentItem.className.indexOf('s-tree-node') === -1) return;
        ev.preventDefault();
    ...
    created() {
      ...
      this.$on('tree-node-drag-over', (event, treeNode) => {
        const dropNode = findNearestComponent(event.target, 'STreeNode');
        ...
        const targetPosition = dropNode.$el.querySelector('.s-tree-node-expand-icon').getBoundingClientRect();
