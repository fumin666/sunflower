<template>
  <span
    ...
    :class="[
      ...
      cascaderSize ? 's-cascader-' + cascaderSize : ''
    ]"
    @click="handleClick"
    ...
    >
      <template slot="suffix">
        <i
          ...
          class="s-input-icon iconfont icon-error"
          @click="clearValue"
        ></i>
        <i
          key="2"
          v-else
          class="s-input-icon iconfont icon-bottom"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      _sFormItemSize() {
        return (this.sFormItem || {}).elFormItemSize;
      },
      cascaderSize() {
        return this.size || this._sFormItemSize || (this.$ELEMENT || {}).size;
      },
      id() {
        return generateId();
      }
    ...
    methods: {
      ...
      handleKeydown(e) {
        ...
        if (keyCode === 13) {
          ...
          setTimeout(() => {
            const firstMenu = this.popperElm.querySelectorAll('.s-cascader-menu')[0];
