<script>
  import { isDef } from '../extra/utils/shared';
  ...
  export default {
    ...
    render(h) {
      ...
      const menus = this._l(activeOptions, (menu, menuIndex) => {
        ...
        const items = this._l(menu, item => {
          ...
          return (
            <li
              class={{
                's-cascader-menu-item': true,
                's-cascader-menu-item-extensible': item.children,
          ...
          >
            {item.label}
            <i
              class={{'iconfont icon-correct-bold':!item.children && item.value === activeValue[menuIndex]}}>
            </i>
            <i
              class={{'iconfont icon-arrow-right':item.children && item.value === activeValue[menuIndex]}}>
            </i>
          </li>
          );
        });
        ...
        return (
          <ul
            ...
            id = { menuId }
          >
            <s-scrollbar wrap-style="max-height: 204px;">
              {items}
              {
                isHoveredMenu
                ? (<svg
      ...
      return (
        <transition name="s-zoom-in-top" on-before-enter={this.handleMenuEnter} on-after-leave={this.handleMenuLeave}>
          <div
            v-show={visible}
            class={[
              's-cascader-menus s-popper',
              popperClass
            ]}
            ref="wrapper"
          >
            <div x-arrow class="popper-arrow"></div>
            {menus}
          </div>
        </transition>
      );
