...
const oldDoLayoutHandler = sTable.methods.doLayout
sTable.methods.doLayout = function (...arg) {
...
export default {
  ...
  computed: {
    ...
    virtualBodyHeight () {
      let data = null;
      if (this.store.table.currentPage && this.store.table.pageSize) {
        let from = this.store.table.pageSize * (this.store.table.currentPage - 1);
        let to = from + this.store.table.pageSize;
        data = this.store.states.data.slice(from, to);
      } else {
  ...
  methods: {
    bindEvent (action) {
      const tableBodyWrapper = this.$el.querySelector('.s-table-body-wrapper .s-scrollbar-wrap')
