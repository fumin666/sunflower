...
import { hasClass, addClass, removeClass, getStyle } from '../extra/utils/dom';
...
export default {
  ...
  props: {
    ...
    fixed: String,
    highlight: Boolean,
    pageSize: [Object, Number],
    currentPage: [Object, Number]
  },
        ...
        <tbody>
          {
            this._l(this.data, (row, $index) =>
              [<tr
                ...
                {
                  !this.fixed && this.layout.scrollX && this.layout.gutterWidth ?
                    <td class="gutter"  style={{ width: this.layout.scrollX ? this.layout.gutterWidth + 'px' : '0' }}/> : ''
                }
              </tr>,
              ...
              <s-tooltip effect={ this.table.tooltipEffect } popperClass="s-table-tooltip" placement="top" ref="tooltip" content={ this.tooltipContent }></s-tooltip>
            )
          }
        </tbody>
      </table>
  ...
  computed: {
    ...
    data() {
      if (this.table.currentPage && this.table.pageSize) {
        ...
        return this.store.states.data.slice(from, to);
      } else {
  ...
  methods: {
    ...
    handleCellMouseEnter(event, row) {
        ...
        if (!(hasClass(cellChild, 's-tooltip') && cellChild.childNodes.length)) {
            return;
        }
    ...
    handleCellMouseLeave(event) {
      ...
      if (tooltip) {
        ...
        if(this.leaveSettime) {
          clearTimeout(this.leaveSettime)
        }
        if(this.endterSettime) {
          clearTimeout(this.endterSettime)
        }
        this.leaveSettime = setTimeout(() => {
          tooltip.handleClosePopper();
        }, 0);
      }
      const cell = getCell(event);
